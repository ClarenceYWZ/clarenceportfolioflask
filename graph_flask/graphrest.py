...
class GraphSession(object):
    ...
    def __init__(self, **kwargs):
        """Initialize instance with default values and users-provided overrides.
        ...
        cache_state = whether to cache session state in local state.json file
                      ...
                      cached, the token will be used without any users
    ...
    def login(self, login_redirect=None):
        """Ask users to authenticate via Azure Active Directory.
        Optional login_redirect argument is route to redirect to after users
        is authenticated.
        """
        if login_redirect:
